...
const INIT_STATE = {
  ...
  isLogout: false,
  isLoading:
    JSON.parse(sessionStorage.getItem("@status")) === true ? false : true,
  isLoggedIn:
    JSON.parse(sessionStorage.getItem("@status")) === true ? true : false,
...
const userReducer = (state = INIT_STATE, action) => {
  switch (action.type) {
    ...
    case ActionType.ADD_SIGNUPEMP_REQUEST: {
      return {
        ...state,
        isLoading: true,
      };
    }
    case ActionType.ADD_SIGNUPEMP_SUCCEED: {
      return applyAddSignupEmpSucceed(state, action);
    }
...
const applyAddSignupEmpSucceed = (state, action) => {
  const { payload } = action;
