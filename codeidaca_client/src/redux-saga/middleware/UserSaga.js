...
function* handleSignupEmp(action) {
  const { payload } = action;
  try {
    const result = yield call(apiUser.signupemp, payload);
    yield put(doSignupEmpSucceed(result.data));
  } catch (error) {
    yield put(doSignupEmpFailed(error));
  }
}
...
function* handleSignin(action) {
  ...
  try {
    ...
    if (Object.keys(result.data.profile).length === 0) {
      yield put(
        doShowAuthMessage({ message: result.data.message, type: "error" })
      );
    } else {
...
export { handleSignup, handleSignupEmp, handleSignin, handleSignout };
